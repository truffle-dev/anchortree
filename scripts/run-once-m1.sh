...
# runs the `webarena_capture` example (which now inlines response bodies) to
...
#
# Exit 0 means M=1 was recorded; any non-zero exit means the run did not prove it.
...
SITE_DIR="$REPO_ROOT/scripts/fixtures/m1-site"
...
echo "==> replaying the HAR with no live origin (M=1 proof)"
ANCHORTREE_CDP_HTTP="http://127.0.0.1:${CDP_PORT}" \
ANCHORTREE_REPLAY_HAR="$HAR_FILE" \
ANCHORTREE_REPLAY_URL="$CAPTURE_URL" \
  cargo run -q -p anchortree-cdp --example webarena_replay
