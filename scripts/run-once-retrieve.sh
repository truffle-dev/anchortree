...
# The filtered review grid: base64("detail=disappointed") as a PATH segment.
FILTER_B64="ZGV0YWlsPWRpc2FwcG9pbnRlZA=="
GRID_URL="${SITE_BASE}/admin/review/product/index/filter/${FILTER_B64}/"
...
# so /admin serves 200 under http://at-sa/. Idempotent: safe on a reused site.
echo "==> pinning Magento base_url to $SITE_BASE/ and flushing cache"
...
" || echo "    (base_url pin skipped; continuing)"
...
echo "==> waiting for $LOGIN_URL to answer (site warm-up can take ~1-2 min)"
for _ in $(seq 1 180); do
  curl -sf "$LOGIN_URL" >/dev/null 2>&1 && break
  sleep 1
done
...
echo "    itself rendered (6 records found), emitted retrieved_data=[6], and the"
echo "    upstream scorer agreed — with its own evaluator + data checksums stamped."
